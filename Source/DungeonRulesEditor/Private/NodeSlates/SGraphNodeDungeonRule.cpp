...
void SGraphNodeDungeonRuleConduit::Construct(const FArguments& InArgs, URuleConduitNode* InNode)
{
...
FSlateColor SGraphNodeDungeonRuleConduit::GetBorderBackgroundColor_Internal(FLinearColor InactiveStateColor, FLinearColor ActiveStateColorDim, FLinearColor ActiveStateColorBright) const
{
	...
	return SGraphNodeDungeonRule::GetBorderBackgroundColor_Internal(FLinearColor(0.38f, 0.45f, 0.21f), ActiveStateColorDim, ActiveStateColorBright);
}

FText SGraphNodeDungeonRuleConduit::GetPreviewCornerText() const
{
	URuleNodeBase* StateNode = CastChecked<URuleNodeBase>(GraphNode);

	return FText::Format(NSLOCTEXT("SGraphNodeDungeonRule", "PreviewCornerConduitText", "{0} conduit"), FText::FromString(StateNode->GetStateName()));
}

const FSlateBrush* SGraphNodeDungeonRuleConduit::GetNameIcon() const
{
